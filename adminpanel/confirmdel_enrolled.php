...
$enrolled_delete = mysqli_query($conn, "SELECT * FROM enrolled WHERE id = '$user_id' ");
	...
	while($row_delete = mysqli_fetch_assoc($enrolled_delete)) {
        ...
        $rcode = $row_delete['reg_code'];
        $date = $row_delete['date'];
	}
...
<a href='enrolled.php'><input type="button" class="form-control" value="NO" style="width: 60px; float: right; margin-right: 45%;"></a>
